---
layout: page
title: 이승윤
description: OPA Leader
img: assets/img/leader/15-lee-seungyoon.png
importance: 15
category: Leader
related_publications: false
---

<div style="display: flex; align-items: center; gap: 2rem; margin-bottom: 2rem;">
  <img src="/assets/img/leader/15-lee-seungyoon.png" alt="이승윤 프로필 사진" style="width: 160px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);" />
  <div>
    <h2 style="margin-bottom: 0.5rem;">이승윤</h2>
    <p style="margin: 0;">
      <a href="https://github.com/RoysyLee" target="_blank"><i class="fab fa-github"></i> GitHub</a>
    </p>
  </div>
</div>

---

### 🛠️ 전문분야
- DevOps, SaaS, Cloud Architecture

### 🤝 활동 커뮤니티
- [OPA (Open cloud Platform Alliance)](https://opakorea.org)

### 🎤 세미나
- [[OPEN CLOUD PLATFORM PUB 2025] session3 클라우드 장애 대응, 서비스 지속형 멀티클라우드](https://youtu.be/D4lpJnbdzA4?si=bKc48gyWRuXYNs7m)

---

### 📂 주요 경력
- (현) 두드림시스템 DevOps팀 팀장
- (전) SSG.com BE/인프라 개발
- (전) 한진중공업 채용시스템 개발
