---
layout: page
title: 이석곤
description: OPA Leader
img: assets/img/leader/31-lee-seokgon.png
importance: 31
category: Leader
related_publications: false
---

<div style="display: flex; align-items: center; gap: 2rem; margin-bottom: 2rem;">
  <img src="/assets/img/leader/31-lee-seokgon.png" alt="이석곤 프로필 사진" style="width: 160px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);" />
  <div>
    <h2 style="margin-bottom: 0.5rem;">이석곤</h2>
    <p style="margin: 0;">
      <a href="mailto:leesk55@gmail.com"><i class="fas fa-envelope"></i> leesk55@gmail.com</a> <br>
      <a href="https://github.com/EricSeokgon" target="_blank"><i class="fab fa-github"></i> GitHub</a> &nbsp;|&nbsp;
      <a href="https://www.linkedin.com/in/eric-seokgon-lee-312991aa/" target="_blank"><i class="fab fa-linkedin"></i> LinkedIn</a>
    </p>
  </div>
</div>

---

### 🛠️ 전문분야
- AI·빅데이터 플랫폼, LLM 엔지니어링

### 🤝 활동 커뮤니티
- [OPA (Open cloud Platform Alliance)](https://opakorea.org)
- 한빛 2025 프리뷰어스
- ChatGPT Korea
- LangChain Korea
- Golang Korea

---

### 📂 주요 경력
- (현) ㈜아이알컴퍼니 부설연구소 수석
- (전) 엔컴㈜ 프로젝트팀 차장
- (전) DBCORE 과장
- AI 및 빅데이터 플랫폼 아키텍처 설계 및 구축
- 마이크로서비스 아키텍처(MSA) 및 클라우드 기반 서비스 설계
- 중소벤처기업진흥공단 기금운영시스템 개선 AI 챗봇 구축
- 사업전환 사업지원을 위한 성장경로 및 위험예측 시스템 개발
- 창업성공패키지 사업 지원을 위한 빅데이터 플랫폼 용역

### 🏆 수상
- 🥇 2024 전자정부 프레임워크 컨트리뷰션 감사장
- 🥇 2023 전자정부 프레임워크 컨트리뷰션 감사장
- 🥇 2022 전자정부 프레임워크 컨트리뷰션 감사장
- 🥇 2021 Cisco DevNet Innovators Hackaton 1st Place Winner

### 📚 저서
- [Go 성능 최적화 가이드](https://product.kyobobook.co.kr/detail/S000208953343) Golang Korea(공동역자)