---
layout: page
title: 허광남
description: eGovFrame Leader
img: assets/img/leader/08-heo-gwangnam.jpg
importance: 8
category: Leader
related_publications: false
---

<div style="display: flex; align-items: center; gap: 2rem; margin-bottom: 2rem;">
  <img src="/assets/img/leader/08-heo-gwangnam.jpg" alt="허광남 프로필 사진" style="width: 160px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);" />
  <div>
    <h2 style="margin-bottom: 0.5rem;">허광남</h2>
    <p style="margin: 0;">
      <a href="https://github.com/kenu" target="_blank"><i class="fab fa-github"></i> GitHub</a>
    </p>
  </div>
</div>

---

### 🛠️ 전문분야
- Node.js 기반의 웹 개발, 협업 및 이커머스 개발 방법론

### 🤝 활동 커뮤니티
- [표준프레임워크 오픈커뮤니티](https://open.egovframe.org)
- [OKKY.kr](https://okky.kr)
- Elasticsearch Korea

---

### 📂 주요 경력
- OKKY공동대표(현재)
- 펫프렌즈 CTO (2019)
- 헬로네이처 CTO (2018)
- GSSHOP 벤처투자팀 CoE 1기 (2015~)
- OKKY(OKJSP) 창립자 (2000~)

### 📚 저서
- 나는 프로그래머다 공저
- [플랫폼을 말하다](https://product.kyobobook.co.kr/detail/S000001622634) 공저
