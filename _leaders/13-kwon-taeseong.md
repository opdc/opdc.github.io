---
layout: page
title: 권태성
description: eGovFrame / OPA Leader
img: assets/img/leader/13-kwon-taeseong.png
importance: 13
category: Leader
related_publications: false
---

<div style="display: flex; align-items: center; gap: 2rem; margin-bottom: 2rem;">
  <img src="/assets/img/leader/13-kwon-taeseong.png" alt="권태성 프로필 사진" style="width: 160px; border-radius: 12px; box-shadow: 0 2px 8px rgba(0,0,0,0.08);" />
  <div>
    <h2 style="margin-bottom: 0.5rem;">권태성</h2>
    <p style="margin: 0;">
      <a href="mailto:taeseong@me.com"><i class="fas fa-envelope"></i> taeseong@me.com</a> <br>
      <a href="https://github.com/taeseong" target="_blank"><i class="fab fa-github"></i> GitHub</a>
    </p>
  </div>
</div>

---

### 🛠️ 전문분야
- Web Application
- SW 품질

### 🤝 활동 커뮤니티
- [표준프레임워크 오픈커뮤니티](https://open.egovframe.org)

### 💻 오픈소스 프로젝트
- 표준프레임워크 컨트리뷰션

---

### 📂 주요 경력
- (주)YH데이타베이스
- CMS, HR 솔루션 등 개발
- 공공정보화사업 프로젝트 다수 참여

### 🏆 수상
- 🏆 2024 표준프레임워크 컨트리뷰션 소스코드 개선 부문 (최우수)
- 🥇 2024 표준프레임워크 컨트리뷰션 개발가이드 개선 부문 (우수)
